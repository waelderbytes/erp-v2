...
|---|---|---|
| Auth/Benutzerverwaltung | ✅ Benutzerverwaltung End-to-End live verifiziert (07.08.2026) | Bootstrap/Login/Refresh, argon2id, Benutzer/Rolle/Berechtigung-Schema inkl. Audit-Log-Trigger (Migration 0001), Kiosk-Login (Migration 0002). Echte Benutzerverwaltung (/benutzer/*, /rollen/*) - Owner/Administrator legen Benutzer an, setzen Passwort/PIN, weisen Rollen zu/entziehen, exklusiv an modul_key 'benutzerverwaltung' gebunden (Migration 0003). Auf dem Server getestet: Benutzer angelegt (kein Hash im JSON - Sicherheitsfix bestaetigt), Rolle 'sachbearbeiter' zugewiesen (alle Berechtigungen korrekt im Response), PIN gesetzt, Rolle wieder entzogen. Ersetzt die frueheren manuellen DB-Updates aus der Zeiterfassung-Entwicklung. Noch offen: RBAC-Guard-Feinabgleich gegen rolle_berechtigung, Selbst-Aussperr-Schutz, REVOKE auf audit_log wirkt nicht gegen den DB-Owner-User (siehe architecture.md Offene Punkte) |
