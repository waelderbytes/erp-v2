...
| Modul | Status | Beschreibung |
|---|---|---|
| Artikelstamm | 🚧 Grundfunktionen + Detailansicht, lokal build-verifiziert | erp-service: Nummernkreis-Engine (einfach + kategoriebasiert XXX-YYY-lfd, aber kategoriebasiertes Schema aktuell TOT - siehe Offene Punkte), Firma-Singleton mit Schema-Sperre, Artikel-CRUD inkl. PATCH (neu), Artikel-Lieferant-Favorit, RBAC scharf (modul_key artikelstamm). Neu: Artikel-Detailseite im Frontend (/artikel/:id) mit Tabs Stammdaten/Bestand/Preise/Lieferanten - Buchungen und Preise direkt am Artikel statt nur ueber separate Modul-Seiten. Anlegen-Formular um beschreibung/einheit/eanGtin/bestandsgefuehrt erweitert. Artikel-/Kunden-/Lieferanten-Tabellen jetzt durchsuchbar, Artikel-Tabelle zusaetzlich sortierbar. Noch offen: i18n bezeichnung, Variantenartikel, kein firma.controller.ts (Artikelnummern-Schema 'kategorie' + Artikelkategorie-Verwaltung im Code vorhanden, aber ohne HTTP-Endpoint nicht nutzbar) |
