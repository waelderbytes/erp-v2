...
| Stammdaten/System-Einstellungen | 🚧 Grundfunktionen implementiert (08.08.2026), lokal build-verifiziert | erp-service: Firmenstammdaten (Migration 0016: Name/Anschrift/USt-IdNr./Steuernummer/Telefon/E-Mail/Kleinunternehmer-Flag § 19 UStG), Steuersaetze (Migration 0014, Seed 19/7/0%), artikel.steuersatz_id jetzt echte Pflicht-FK (Migration 0015, letzte offene Feldkatalog-Luecke geschlossen). Neue Endpoints GET/PATCH /firma, POST /firma/artikelnummern-schema, PATCH /firma/artikelnummern-stellen, GET/POST/PATCH /steuersaetze, GET/PATCH /nummernkreise/:entityKey - alle unter neuem RBAC-modul_key "stammdaten". Frontend: Seite /stammdaten (Tabs Firma/Steuersaetze/Nummernkreise), nur in der Nav fuer Owner/Administrator sichtbar. Nutzerentscheidung: erstmal 1 Firma, Mehrfirmen/Niederlassungen bleibt eigener, spaeterer Umbau (firma_id muesste an Kunde/Lieferant/Artikel/Nummernkreis etc. ran). Noch offen: Live-Verifikation auf dem Server (Migrationen lokal nicht gegen echte DB testbar), eigene RBAC-Rollenzuteilung fuer "stammdaten" ausserhalb Owner/Administrator |
...
| Modul | Status | Beschreibung |
|---|---|---|
| Artikelstamm | 🚧 Assistent (Anlegen+Bearbeiten vereint) + Mehrsprachigkeit, lokal build-verifiziert | erp-service: Nummernkreis-Engine (einfach + kategoriebasiert - seit 08.08.2026 vollstaendig nutzbar: eigenes CRUD fuer Haupt-/Untergruppen unter /artikelkategorien inkl. Vorschau-Endpoint, Frontend-Tab 'Artikel-Warengruppen' + Auswahl im Artikel-Anlegen-Formular), Artikel-CRUD inkl. PATCH, Artikel-Lieferant-Favorit. Neu (nach v1-Vorbild waelderbytes-suite): Mehrsprachigkeit fuer Kurztext/Langtext - 'de' bleibt direkt auf bezeichnung/beschreibung, Zusatzsprachen in neuer Tabelle artikel_uebersetzung (Migration 0008, GET/PUT/DELETE /artikel/:id/uebersetzungen), dazu artikel.interne_notiz (rein intern, nie auf Belegen) und kunde.sprache jetzt auch im Anlegen-DTO nutzbar (Spalte existierte schon). Frontend: kein separater Anlegen-Dialog mehr - Route /artikel/neu nutzt denselben Tab-Screen wie das Bearbeiten (Stammdaten speichern schaltet Bestand/Preise/Lieferanten/Sprachen frei), Sprachen-Tab mit Sprachen-Tabs-UI wie in v1. Neu (08.08.2026): alle Tabs von Anfang an sichtbar (disabled bis Stammdaten gespeichert) + Weiter/Zurueck-Fuehrung mit Auto-Sprung nach dem Speichern (ueberspringt deaktivierte Tabs), Routing-Bugfix 'Artikel neu haengt bei Laedt...', echtes Einheiten-Modul (artikel.einheit_id als FK statt Freitext, eigenes CRUD unter /einheiten) mit Such-/Anlegen-Dropdown (SearchCreateDropdown, domainfrei fuer kuenftige Dropdowns wiederverwendbar), Kurztext-Vorschlaege aus vorhandenen Artikeln waehrend des Tippens, Log-Tab (Audit-Trail + Lagerbuchungen kombiniert, GET /artikel/:id/log), Bestand-Tab immer sichtbar statt ausgeblendet, PWA-Installierbarkeit mit Platzhalter-Icon (vite-plugin-pwa), bomfaehig-Flag (Migration 0011, nur bei fertigungsartikel wirksam, Vorbereitung Stueckliste/BOM). gewicht_kg/Masse/mindestbestand nachgezogen (Migration 0013). Noch offen: i18n-Fallback-Logik beim Beleg-Rendering (Phase 3), Variantenartikel. steuersatz_id ist seit Migration 0015 eine echte Pflicht-FK, firma.controller.ts existiert jetzt (Modul Stammdaten/System-Einstellungen) |
...
| Modul | Status | Beschreibung |
|---|---|---|
| Belegkette (Verkauf) | 🚧 Grundfunktionen implementiert (08.08.2026), lokal build-verifiziert | erp-service: gemeinsames Beleg+Beleg-Position-Modell (Migration 0017) fuer Angebot/Auftragsbestaetigung/Lieferschein/Rechnung - Feldschema an ERP v1 angelehnt, Ablauf/Teillieferungslogik bewusst NEU entworfen (Nutzerentscheidung 08.08.2026: v1 nur fuer Felder, nicht fuer Ablaeufe uebernehmen). Echte Teillieferung/-rechnung pro Position (weitergefuehrte_menge, analog dem bereits bestehenden Bestellposition-Muster aus Einkauf/Bestellwesen), automatischer Status offen/teilweise_weitergefuehrt/abgeschlossen/storniert. Preis-/Steuersatz-Snapshot pro Position (GoBD-Gedanke), Preisfindung wird beim Anlegen automatisch herangezogen falls kein manueller Preis angegeben. Lieferschein-Anlage bucht automatisch Warenausgang (neue Methode warenausgangInTransaktion in lagerbewegung.service.ts). Kleinunternehmer-Flag (firma.entity.ts) steuert die USt-Anzeige. festgeschrieben-Flag fuer GoBD-Unveraenderlichkeit bei Rechnungen (manueller Endpoint, noch nicht an PDF-Erzeugung gekoppelt). Frontend: neue Nav-Gruppe "Vertrieb" (Angebote/Auftraege/Lieferscheine/Rechnungen), generische Listen-/Detail-Komponenten. Seit 08.08.2026 zusaetzlich Proformarechnungen/Abschlagsrechnungen als eigene, ausdruecklich NICHT in die Hauptkette eingebundene Zusatzbelege (BelegService.zusatzbeleg()), entstehen nur aus einer Auftragsbestaetigung, beeinflussen deren weitergefuehrte Menge/Status nicht, abschlag ist wie rechnung festschreibbar, proforma nicht. Noch offen: PDF-Ausgabe (bewusst als Folgeschritt zurueckgestellt), Live-Verifikation auf dem Server (Migration 0017 lokal nicht gegen echte DB testbar), eigene RBAC-Rollenzuteilung fuer modul_key "verkauf" ausserhalb Owner/Administrator |
